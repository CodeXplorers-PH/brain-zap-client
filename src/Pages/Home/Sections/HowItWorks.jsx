  ...
  return (
    <section className="relative bg-gray-900 py-28 px-4 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 right-1/3 w-96 h-96 bg-purple-900/20 rounded-full filter blur-3xl opacity-20 animate-pulse" style={{ animationDuration: '15s' }}></div>
        <div className="absolute bottom-1/3 left-1/4 w-96 h-96 bg-blue-900/20 rounded-full filter blur-3xl opacity-20 animate-pulse" style={{ animationDuration: '12s' }}></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="text-xl text-gray-400 max-w-3xl mx-auto leading-relaxed">
            Experience personalized, AI-driven learning that adapts to your unique journey 
            and transforms the way you acquire and retain knowledge.
          </p>
        </motion.div>
          ...
          >
            {processSteps.map((step, index) => (
              ...
              >
                ...
                {/* Background gradient */}
                <div className={`absolute -left-16 -bottom-16 w-40 h-40 bg-gradient-to-br ${step.gradient} opacity-10 rounded-full blur-xl transition-all duration-500 group-hover:opacity-20 group-hover:scale-125`}></div>
                
                <div className="relative z-10">
                  <div className={`w-12 h-12 bg-gradient-to-br ${step.gradient} rounded-lg flex items-center justify-center mb-6 shadow-lg shadow-purple-900/20 group-hover:scale-110 transition-transform duration-300`}>
                    {step.icon}
                  </div>
                ...
                <div className="absolute bottom-4 right-4 w-3 h-3">
                  <span className={`absolute inline-flex h-full w-full rounded-full bg-gradient-to-r ${step.gradient} opacity-75 animate-ping`}></span>
                  <span className={`relative inline-flex rounded-full h-3 w-3 bg-gradient-to-r ${step.gradient}`}></span>
                </div>
              </motion.div>
            ))}
          </motion.div>
