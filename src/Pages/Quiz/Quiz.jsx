...
const Quiz = ({
  ...
  const handleOptionSelect = (questionIndex, option) => {
    ...
    const nextUnanswered = questions.findIndex(
      (_, idx) => idx > questionIndex && !selectedOptions[idx]
    );
  ...
  return (
    <div className="max-w-5xl mx-auto pb-24 px-4">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {q.options.map((option, i) => (
                <button
                  key={i}
                  className={`p-5 rounded-2xl flex items-start border text-left transition-all duration-200 hover:shadow-md ${
      ...
      <div
        className={`fixed bottom-8 left-1/2 transform -translate-x-1/2 w-full max-w-[90%] ${
          questions.length <= 5
            ? 'sm:max-w-md'
            : questions.length <= 10
            ? 'sm:max-w-lg'
