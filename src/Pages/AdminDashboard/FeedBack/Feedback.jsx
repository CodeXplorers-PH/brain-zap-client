...
const Feedback = () => {
  ...
  const handleMarkAsRead = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.patch(`/feedbackRead/${id}`).then((res) => {
          if (res?.data?.success) {
            Swal.fire({
              title: "Success!",
            ...
            setFeedbacks((prevFeedbacks) =>
              prevFeedbacks.map((fb) => (fb._id === id ? { ...fb, read: "Done" } : fb))
            );
          }
        }).catch((error) => {
  ...
  const handleDeleteFeedback = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/feedbackDelete/${id}`).then((res) => {
          if (res?.data?.success) {
            Swal.fire({
              title: "Deleted!",
  ...
  const filterOptions = ["All", "Feedback", "Feature Request", "Bug Report", "Question", "Unread", "Read"];
  ...
  const filteredFeedbacks = feedbacks.filter(feedback => {
    ...
    if (filterType === "Unread") return matchesSearch && !(feedback.read === true || feedback.read === "Done");
    if (filterType === "Read") return matchesSearch && (feedback.read === true || feedback.read === "Done");
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-400">View and manage user feedback and feature requests</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700/50 p-4">
          ...
          <p className="text-2xl font-bold text-white">{feedbacks.length || 0}</p>
        </div>
        ...
        <div className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700/50 p-4">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <h3 className="text-sm font-medium text-gray-300">Feature Requests</h3>
          </div>
          <p className="text-2xl font-bold text-white">
            {feedbacks.filter(f => f.feedbackType === "Feature Request").length || 0}
          </p>
        </div>
      ...
      <div className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700/50">
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white">Feedback Messages ({filteredFeedbacks.length})</h2>
          </div>
            ...
            {loading ? (
                  ...
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-2 py-1 rounded-full ${
                        feedbackTypeColors[feedback.feedbackType] || 'bg-gray-500'
                      }`}>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-gray-700/50">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        disabled={feedback?.read === true || feedback?.read === "Done"}
                      >
                        <CheckCircle size={16} />
                      </button>
                      <button
              ...
              <div className="text-center py-10">
                ...
                <p className="text-gray-500 text-sm mt-1">
                  {searchQuery ? "Try a different search query or filter" : "All clear! No messages to display"}
                </p>
              </div>
            )}
          </div>
