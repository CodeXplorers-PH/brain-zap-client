      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {pricingPlans.map((plan, index) => (
            ...
            >
              {plan.popular && (
                <div className={`absolute top-0 right-0 ${plan.badgeColor} text-white text-sm font-bold px-4 py-2 rounded-bl-xl rounded-tr-xl`}>
                  Most Popular
                </div>
              )}
              <div className="px-8 py-12 flex flex-col flex-grow">
                <h3 className={`font-secondary text-4xl font-bold ${plan.textColor}`}>
                  {plan.title}
                </h3>
                <p className={`mt-2 ${plan.descColor}`}>{plan.description}</p>
                <div className="mt-6">
                  <span className={`text-5xl font-semibold ${plan.priceColor}`}>
                    {plan.price}
                  </span>
                ...
                <ul className="mt-8 space-y-4 flex-grow">
                  {plan.features.map((feature, idx) => (
                    <li key={idx} className="flex items-center">
                      ...
                      <span className={`ml-3 ${plan.textColor}`}>{feature}</span>
                    </li>
                  ))}
                </ul>
                <button
