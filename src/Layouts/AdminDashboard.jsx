...
const AdminDashboard = () => {
  ...
  const navItems = [
    { to: "/dashboard/adminHome", icon: <House size={20} />, label: "Admin Home" },
    { to: "/dashboard/feedback", icon: <MailCheck size={20} />, label: "Feedback" },
    { to: "/dashboard/allUsers", icon: <Users size={20} />, label: "All Users" },
  ];

  const secondaryNavItems = [
    { to: "/", icon: <House size={20} />, label: "Home" },
    { to: "/start-quiz", icon: <MdOutlineQuiz className="text-lg" />, label: "Start Quiz" },
      ...
      >
        <div className="min-h-full bg-gray-800 border-r border-gray-700/50 flex flex-col relative">
          ...
          <div className="md:hidden flex justify-end p-4">
            <button onClick={handleToggleSidebar} className="text-gray-400 hover:text-white">
              <X size={24} />
            </button>
          </div>

          {/* Logo */}
          <div className={`flex items-center justify-center py-6 ${isCollapsed ? "px-2" : "px-6"}`}>
            {!isCollapsed && (
              <span className="text-2xl lg:pl-2 font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-400">BrainZap</span>
            )}
          </div>

          {/* User info */}
          <div className={`px-4 py-4 ${isCollapsed ? "items-center" : ""} flex ${isCollapsed ? "justify-center" : "items-center"} border-b border-gray-700/50`}>
          ...
          <div className="flex-1 overflow-y-auto py-4 px-3">
            ...
            <div className={`mt-8 mb-2 px-4 ${isCollapsed ? "text-center" : ""}`}>
              <p className="text-xs uppercase tracking-wider text-gray-500 font-medium">
            ...
            <ul className="space-y-1">
              {secondaryNavItems.map((item, index) => (
                <li key={index}>
                  <NavLink
                    to={item.to}
                    className={`flex items-center ${isCollapsed ? "justify-center" : ""} px-4 py-3 rounded-lg transition-colors duration-200 text-gray-400 hover:text-white hover:bg-gray-700/50`}
                    title={isCollapsed ? item.label : ""}
                  >
                    <span className="flex-shrink-0">{item.icon}</span>
                    {!isCollapsed && <span className="ml-3">{item.label}</span>}
